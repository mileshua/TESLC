...
with open("solar.csv") as solar:
    ...
    for row in reader:
        ...
        if row[1] not in data_carbon:
            ...
            x.append(float(row[7].replace(",", ""))/int(row[5].replace(",", "")))
...
for i, txt in enumerate([a.split("(")[0].strip() for a in x_label]):
    ax.annotate(txt, (x[i], y[i]))
m, b, r_value, p_value, std_err = scipy.stats.linregress(x, y)

# ax.plot(x, m*np.array(x)+ b)
ax.set_xlabel("Solar Generation Per Capita")
